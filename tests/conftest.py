...
def _detect_pdf_mode() -> tuple[str, Path | None, Path | None]:
    """
    Detect which PDF set is available and return mode + paths.

    Returns:
        Tuple of (mode, good_pdfs_dir, bad_pdfs_dir) where:
        - mode: "anonymised" if symlinks exist, "bundled" if PDFs found, "none" otherwise
        - good_pdfs_dir: Path to good PDFs directory (or None if not found)
        - bad_pdfs_dir: Path to bad PDFs directory (or None if not found)
    """
    ...
    if (
        fallback_good is not None
        and fallback_bad is not None
        and list(fallback_good.glob("*.pdf"))
        and list(fallback_bad.glob("*.pdf"))
    ):
...
if PDF_MODE == "anonymised":
    print("[PDF_FIXTURES] Using ANONYMISED PDFs (symlinks detected)")
    print(f"[PDF_FIXTURES] Location: {_GOOD_PDF_DIR_OVERRIDE}")
elif PDF_MODE == "bundled":
    print("[PDF_FIXTURES] Using BUNDLED PDFs from installed package")
else:
    print("[PDF_FIXTURES] No PDFs available (tests will skip)")
...
def openstan_env(bsp_harness: TestHarness) -> Generator[OpenStanEnv, None, None]:
    ...
    if (
        PDF_MODE == "none"
        or _GOOD_PDF_DIR_OVERRIDE is None
        or _BAD_PDF_DIR_OVERRIDE is None
    ):
        pytest.skip(
