...
# On Linux CI the PNG is always generated before this script runs (see
# release.yml "Convert app icon SVG → PNG" step).  Include it
# unconditionally so a missing file causes an immediate loud failure
# rather than silently producing a package without an icon.
if sys.platform != "win32" and sys.platform != "darwin":
    if not _ICON_PNG.exists():
        raise FileNotFoundError(
            f"Icon PNG not found at {_ICON_PNG}. "
            "Run the 'Convert app icon SVG → PNG' step before building."
        )
